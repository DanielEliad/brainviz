...
// Parameters for group average data fetching
export type GroupAverageParams = {
  group: "ASD" | "HC" | null;
  method: CorrelationMethod | null;
  windowSize: number;
  step: number;
...
const DEFAULT_GROUP_PARAMS: GroupAverageParams = {
  group: null,
  method: null,
  windowSize: 30,
  step: 1,
...
export function useGroupAverageData(params: Partial<GroupAverageParams> = {}) {
  const p = { ...DEFAULT_GROUP_PARAMS, ...params };
  const [time, setTime] = useState<number>(0);
  ...
  const dataQuery = useQuery<GraphDataResponse>({
    ...
    queryFn: async () => {
      if (!p.group) throw new Error("No group selected");
      if (!p.method) throw new Error("No correlation method selected");
      ...
      const res = await fetch(`${API_URL}/abide/group-data`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      if (!res.ok) {
        const text = await res.text();
        throw new Error(`Group data fetch failed (${res.status}): ${text}`);
      }
      return res.json();
    },
    enabled: !!p.group && !!p.method,
    retry: 1,
  });
